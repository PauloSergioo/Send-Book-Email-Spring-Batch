...
@Configuration
public class ProcessLoanNotificationProcessorConfig {

    @Bean
    public ItemProcessor<UserBookLoan, Mail> processLoanNotificationProcessor() {

        return new ItemProcessor<UserBookLoan, Mail>() {
            @Override
            public Mail process(UserBookLoan loan) throws Exception {
                Email from = new Email("paulosergioilha38@gmail.com", "Biblioteca Municipal");
